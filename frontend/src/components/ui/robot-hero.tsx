...
export function ResponsiveGroup({ children }: { children: React.ReactNode }) {
...
export function RobotPrototype({ neckParams = { baseR: 0.25, baseH: -0.01, midR: 0.23, midH: 0.02, lipBottomR: 0.27, lipBottomH: 0.045, lipTopR: 0.27, lipTopH: 0.055, innerR: 0.1, innerDropH: 0.0 }, bodyParams = { bodyBevelR: 0.21, bodyBevelY: 0.38, bodyBevelT: 0.015 }, moveRange = 1 }: {
  ...
  useFrame((state, delta) => {
    ...
    const maxMoveX = (state.viewport.width / 3.5) * moveRange;
...
export function RobotHero({
  ...
  return (
    <section
      ref={containerRef}
      className={`relative w-full overflow-hidden ${className || 'h-dvh min-h-[600px]'}`}
